...
if not st.session_state.logged_in:
    ...
    with _lc:
        ...
        # Store login credentials accordion — shows username hints only, no plaintext passwords
        with st.expander("📋 Store Login Credentials", expanded=False):
            ...
            <div style="font-size:0.76rem;color:#4a6a8a;margin-bottom:10px">
              ...
              Passwords are not shown here for security — contact your administrator if you need access.
            </div>""", unsafe_allow_html=True)
            _actual_creds = [
                ("🏪", "MedPlus Ameerpet",      "medplus_store"),
                ("🏥", "Apollo Kukatpally",      "apollo_store"),
                ("🩺", "WellCare Banjara Hills", "wellcare_store"),
                ("💉", "HealthFirst Madhapur",   "healthfirst_store"),
                ("🧬", "CityMed Dilsukhnagar",   "citymed_store"),
            ]
            for _ico, _sname, _uname in _actual_creds:
                st.markdown(f"""
                <div style="display:flex;align-items:center;justify-content:space-between;
                  ...
                  <div style="display:flex;gap:8px">
                    ...
                    <span style="background:#fce4ec;color:#880e4f;font-size:0.70rem;font-weight:700;
                                padding:3px 9px;border-radius:6px">••••••••</span>
                  </div>
                </div>""", unsafe_allow_html=True)
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    sales = sales.merge(
        prod[["ProductID", "UnitCost", "RetailPrice"]].rename(
            columns={"RetailPrice": "UnitPrice"}),
...
Top Risky Supplier: {_cop_sup.iloc[0]['Supplier_Name'] if len(_cop_sup)>0 else 'N/A'} (Risk: {_cop_sup.iloc[0]['RiskScore']:.0f} if len(_cop_sup)>0 else 0)
